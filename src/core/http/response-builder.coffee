...
module.exports = (jsonSchemaValidator) ->
  ...
  class Response
    ...
    paging: (nextPageUrl, isLastPage = false) ->
      @paging =
        continue: !isLastPage
    ...
    send: (res) ->
      ...
      if @isSuccess and typeof(response.data) == 'object' and Object.keys(response.data).length == 0 and response.data not instanceof Array
        statusCode = 204
        response = null
  ...
  responseBuilder =
    ...
    responseError: (type, message) ->
      return new ResponseError(type, message)
    propertyError: (propertyName, errorMessage) ->
      return new PropertyError(propertyName, errorMessage)
